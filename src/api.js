...
export const GETSUGESTIONS = async () =>{
    ...
    .catch(err => err)
    return response.data.data.sugestions
}

export const CREATESUGESTION = async (title) =>{

    const response = await axios.post(process.env.REACT_APP_API_URL,{
        query:`mutation{
            createSugestion(title:"${title}"){
                title
            }
        }`,
        headers: {
    ...
    return response.data.data.sugestion
}
